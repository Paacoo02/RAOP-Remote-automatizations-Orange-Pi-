...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  // -----------------------------------------------------------------
  //  INICIO: LÓGICA MODIFICADA PARA CELDA 2 (CORREGIDA)
  // -----------------------------------------------------------------
  ...
  console.log('⏳ Esperando a que aparezca el diálogo de montaje (o se automonte)...'); 
  await sleep(2000); // 2s de gracia para que la UI reaccione

  // Esperamos 10 segundos por el botón. Si no aparece, 'focused' será false.
  const focused = await waitAndFocusConnectButton(colabPage, 10000); 

  if (focused) {
      console.log('✅ Botón "Conectar con Google Drive" detectado y enfocado.');
      await colabPage.keyboard?.press('Enter');
      console.log('↩️ ENTER enviado al diálogo.');

      // ⬇ Manejo de popups OAuth (SOLO OCURRE SI SE PULSÓ ENTER)
      await waitForOAuthCascade(context, colabPage, handleOAuthPopupByEmailOrForm, { windowMs: 120000, idleMs: 2000 });
      await colabPage.bringToFront?.();
  } else {
      console.log('ℹ️ No se detectó el diálogo "Conectar" en 10s (probablemente ya montado).');
      // No presionamos ENTER.
      // No llamamos a waitForOAuthCascade.
  }
