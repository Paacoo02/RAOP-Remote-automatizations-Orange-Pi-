...
# ======================================================
playwright_ensure() {
  echo "🔎 Verificando navegadores (Build instaló 1194, App busca 1193)..."
  
  local TARGET_DIR="${PLAYWRIGHT_BROWSERS_PATH}/chromium-1193"
  local ACTUAL_DIR="${PLAYWRIGHT_BROWSERS_PATH}/chromium-1194"
  local ACTUAL_EXE="${ACTUAL_DIR}/chrome-linux/chrome"

  # 1. Comprobar si el ejecutable real (1194) existe
  if ! [ -f "${ACTUAL_EXE}" ]; then
    echo " "
    echo "❌ FALLO CRÍTICO DE BUILD: No se encontró ${ACTUAL_EXE}."
  ...
  # 2. Comprobar si el enlace (1193) ya existe
  if [ -L "${TARGET_DIR}" ] || [ -d "${TARGET_DIR}" ]; then
    echo "✅ 'chromium-1193' (o un enlace) ya existe. ¡Todo listo!"
    return 0
  fi

  # 3. Crear el enlace
  echo "🔗 'chromium-1194' encontrado, pero la app busca 'chromium-1193'."
  echo "   Creando enlace simbólico: ${TARGET_DIR} -> ${ACTUAL_DIR}"
  ln -s "${ACTUAL_DIR}" "${TARGET_DIR}"
  
  if [ -L "${TARGET_DIR}" ]; then
    echo "✅ Enlace creado con éxito. La app será redirigida."
  else
    echo "❌ FALLO: No se pudo crear el enlace simbólico."
    exit 1
  fi
}
