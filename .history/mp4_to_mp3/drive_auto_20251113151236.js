...
const {
  ...
  FIXED_FOLDER_URL,
  waitForDriveReady // <-- IMPORTANTE: Añadir esta importación
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  // Damos 2 segundos de gracia para que la UI reaccione y muestre el botón
  await sleep(2000); 

  // Esperamos MÁXIMO 10 segundos por el botón. Si no aparece, 'focused' será false.
    ...
    for (let attempt = 1; attempt <= MAX_DOWNLOAD_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt > 1) {
          ...
          // Usar goto() es más estable que reload()
          await videosTab.goto(FIXED_FOLDER_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
