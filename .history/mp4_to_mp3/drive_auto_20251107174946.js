  ...
  try {
    while (Date.now() < deadline) {
      ...
      for (const p of candidates) {
        try { await p.waitForLoadState('domcontentloaded', { timeout: detectTimeout }).catch(()=>{}); } catch {}
        const url = p.url();
...
async function drive_auto() {
  ...
  // 2) Abrir notebook (forzado en inglés con &hl=en arriba)
  ...
  // ⬇️ NUEVO: esperar enlace trycloudflare O "True"
