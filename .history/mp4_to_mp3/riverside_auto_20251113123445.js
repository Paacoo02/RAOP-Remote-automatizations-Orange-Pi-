  ...
  for (const sel of [
    ...
    try {
      ...
      if (await btn.count()) {
        ...
        await page.waitForTimeout(100); // <-- Optimización: 300 -> 100
        break;
      }
    } catch {}
  }
...
async function clickTranscribeNowAndWaitUploadUI(page) {
  ...
  for (let round = 1; round <= 3; round++) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await page.waitForTimeout(300); // <-- Polling, 300ms está bien
    }
  }
...
async function setLanguageTo(page, target = "spanish") {
  ...
  await page.waitForTimeout(100); // <-- Optimización: 200 -> 100
...
async function markConsentCheckboxEnhanced(page) {
  ...
  await page.waitForTimeout(150).catch(()=>{}); // <-- Optimización: 250 -> 150
  ...
  try {
    ...
    if (has) {
      await ifr
        ...
        .click({ timeout: 3000 })
        .catch(()=>{});
      await page.waitForTimeout(150); // <-- Optimización: 500 -> 150
    }
  } catch {}
...
async function activateStartButton(page) {
  ...
  await page.waitForTimeout(250); // <-- Optimización: 600 -> 250
...
async function waitUploadProcessed(page, { totalTimeout = UPLOAD_TOTAL_TIMEOUT, stuckTimeout = UPLOAD_STUCK_TIMEOUT } = {}) {
  ...
  while (Date.now() - t0 < totalTimeout) {
    ...
    await page.waitForTimeout(500); // <-- Polling, 500ms está bien
  }
...
async function waitForTranslateAndDownloadReady(page, { timeoutMs = 0 } = {}) {
  ...
  const ok = await page.waitForFunction(
    ...
    { timeout: timeoutMs, polling: 'raf' } // <-- polling: 'raf' es óptimo
  ).then(() => true).catch(() => false);
...
async function waitForReadyUI(page, { timeoutMs = READY_WAIT_TIMEOUT_MS, dumpOnReady = true } = {}) {
  ...
  const ok = await page
    ...
    }, { timeout: timeoutMs, polling: 300 }) // <-- Optimización: 800 -> 300
    .then(() => true)
    .catch(() => false);
...
async function transcribeFromTmpOrPath({
  ...
  try {
    ...
    const fileSet = await runUntilTrue(
      [
        ["Filechooser + click en '+'", async () => {
          ...
          if (clicked) await page.waitForTimeout(200); // <-- Optimización: 800 -> 200
    ...
    await page.waitForTimeout(300); // <-- Optimización: 1200 -> 300
