// app.js (versión unificada + envío de salida a Gemini)
...
app.post("/match", upload.single("file"), async (req, res) => {
  try {
    ...
    if (isVideo) {
      ...
      // 4) 🚀 NUEVO: Transcribir en Riverside
      ...
      // 5) 🚀 NUEVO: Pasar salida de Riverside a Gemini para resumen
      ...
      try {
        ...
        gemini = await summarizeWithGemini({
          prompt,
          text: riverside?.transcript || "", // preferimos texto de Riverside
          // Si quieres que Gemini use el audio directamente, además pasa:
          audioPath: mp3Local
        });
      } catch (e) {
      ...
      return res.json({
        ...
        riverside, // incluye transcript/transcriptUrl/jobId si tu módulo los devuelve
        gemini     // incluye summary/link si tu módulo los devuelve
      });
    }

    // ==== Flujo de imagen (sin cambios) ====
