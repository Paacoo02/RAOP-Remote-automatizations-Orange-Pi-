// drive_auto.js — Abre Colab, monta Drive, espera link/True y (solo si True) descarga/borra video.mp3
'use strict';
...
// ⭐ NUEVO: leer JSON de la salida de una celda concreta
async function readJsonFromCellOutput(page, idx = 1) {
  return await page.evaluate((i) => {
    ...
    // 1) Intento principal: encontrar un bloque { ... } multilínea
    const extractJsonBlock = (src, openChar, closeChar) => {
      const start = src.indexOf(openChar);
      const end = src.lastIndexOf(closeChar);
      if (start === -1 || end === -1 || end <= start) return null;
      return src.slice(start, end + 1).trim();
    };

    let candidate = extractJsonBlock(joined, '{', '}');
    if (!candidate) {
      // También probamos con array JSON
      candidate = extractJsonBlock(joined, '[', ']');
    }
    ...
    // 2) Fallback: buscar líneas que parezcan JSON "limpio"
    ...
    for (let idxLine = lineCandidates.length - 1; idxLine >= 0; idxLine--) {
      const parsed = tryParse(lineCandidates[idxLine]);
      if (parsed !== null) return parsed;
    }
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  console.log('2️⃣ Ejecutando Celda 2 (montaje + conversión)…');
  try {
  ...
  // 1) Esperamos a ver "Mounted at /content/drive" (si aparece)
  await waitForMountedDriveInCell(colabPage, 1, { timeoutMs: 180000 }).catch(() => {
    console.warn('⚠️ No vimos explícitamente "Mounted at /content/drive"; continuamos igualmente.');
  });
  console.log('✅ Detectado (o asumido) montaje de Drive; esperando fin completo de la celda 2…');

  // 2) Ahora sí esperamos a que la celda termine por completo (incluye ffmpeg + copia + print JSON)
  await waitForCellToFinish(colabPage, 1, { timeoutMs: 600000 }).catch((e) => {
    console.warn('⚠️ La celda 2 no terminó dentro del tiempo esperado:', e?.message || e);
  });

  console.log('✅ Celda 2 completada, intentando leer JSON…');

  // Leer el JSON devuelto por la celda 2 (idx = 1)
  try {
    cell2Json = await readJsonFromCellOutput(colabPage, 1);
    if (cell2Json) {
      console.log('\n📦 JSON devuelto por la celda 2:\n', JSON.stringify(cell2Json, null, 2));
    } else {
      console.log('ℹ️ Celda 2 no devolvió un JSON parseable.');
    }
  } catch (e) {
    console.warn('⚠️ Error al intentar leer JSON de la celda 2:', e.message);
  }
  ...
  if (outcome?.kind === 'link') {
    ...
    return { result: outcome.value, page: colabPage, context, cell2Json };
  }
...
if (outcome?.kind === 'true') {
  ...
  for (let attempt = 1; attempt <= MAX_DOWNLOAD_ATTEMPTS; attempt++) {
    try {
      ...
      if (attempt > 1) {
        ...
        await videosTab.reload({ waitUntil: 'domcontentloaded', timeout: 60000 }).catch(e => console.warn(`Error recargando videosTab: ${e.message}`));
        await videosTab.locator('div[role="main"]').first().waitFor({ state: 'visible', timeout: 30000 }).catch(()=>{});
        await sleep(2000); 
      }
  ...
  return { result: true, download: dlInfo, page: colabPage, context, cell2Json }; // <-- Adjuntamos JSON
}
...
drive_auto()
  .then(({ result, cell2Json }) => { // <-- Extraemos el nuevo JSON
    console.log('\n📊 RESULTADO FINAL (Celda 3):\n', result);
    
    if (cell2Json) {
      console.log('\n📦 DATOS (Celda 2 JSON):\n', JSON.stringify(cell2Json, null, 2)); // <-- Lo imprimimos
    } else {
