...
function scanOnce() {
  ...
  try {
      ...
      if (!s || s.startsWith('about:blank') || s.startsWith('javascript:')) {
        ...
        if (guess?.url) {
          ...
          if (!sentUrls.has(u)) {
            ...
            chrome.runtime.sendMessage({ type:'videoFound', url:u, via:'embed-lazy-guess', frameUrl: location.href, meta: guess.meta }).catch(()=>{});
          }
    ...
    lazyElements.forEach(el => {
      ...
      chrome.runtime.sendMessage({ type: 'dbg.ping', from: 'scanOnce_SENDING_videoFound', extra: { via: 'embed-youtube-lazy', src: s } }).catch(()=>{});
      sentUrls.add(s);
      chrome.runtime.sendMessage({ 
        type:'videoFound', 
        url:s, 
        ...
        frameUrl: location.href, 
        meta: { provider:'yt', providerId: videoId, thumb:`https://img.youtube.com/vi/${videoId}/hqdefault.jpg` }
      }).catch(()=>{});
    });
...
try {
  scanObserver.observe(document.documentElement || document, { 
    ...
    attributeFilter: ['src','srcdoc','poster','data-src','data-lazy-src','data-embed','data-url','data-href','data-videoid','data-ytid','data-yt-id']
  });
} catch(e) {
