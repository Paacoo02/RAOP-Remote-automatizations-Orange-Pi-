...
function scanOnce() {
  ...
  try {
    ...
    // ####################################################################
    // ### INICIO DE LA MODIFICACIÓN: BUSCADOR DE LAZY-LOAD ###
    // ####################################################################
    
    // Busca elementos que tengan atributos de datos con el ID de YouTube
    // Esto es para páginas que "esconden" el ID y lo cargan con JS
    document.querySelectorAll('[data-videoid], [data-ytid], [data-yt-id]').forEach(el => {
      const videoId = el.dataset.videoid || el.dataset.ytid || el.dataset.ytId;

      // Si no hay ID, o el ID es 'false' (a veces pasa), saltar
      if (!videoId || videoId === 'false') {
        return;
      }
      
      // Construir la URL nosotros mismos. Usamos youtube-nocookie.com por defecto.
      const s = `https://www.youtube-nocookie.com/embed/${videoId}`;

      // Si ya la hemos enviado (porque el iframe se cargó rápido), saltar
      if (sentUrls.has(s)) {
        return;
      }
