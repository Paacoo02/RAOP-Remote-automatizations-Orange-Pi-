...
const dbgDumpDOMBtn = document.getElementById('dbgDumpDOM'); // Botón nuevo
...
// Lógica del botón nuevo para Volcar DOM
dbgDumpDOMBtn.onclick = async () => {
  const tab = await activeTab();
  if (!tab) return alert('No hay pestaña activa.');
  
  dbgDumpDOMBtn.disabled = true;
  dbgDumpDOMBtn.textContent = 'Volcando...';
  
  try {
    // Enviar el mensaje al content.js de la pestaña activa
    const res = await chrome.tabs.sendMessage(tab.id, { type: 'dbg.dumpDOM' });
    alert('Petición de DOM enviada. Revisa tu terminal de logger_server.js');
    console.log('Respuesta de volcado de DOM:', res);
  } catch (e) {
    console.error('Fallo al enviar petición de volcado de DOM', e);
    alert('Fallo al enviar la petición de volcado de DOM.\n\n¿Has recargado la página después de instalar la extensión?\n\n' + (e?.message || String(e)));
  }
