...
function scanOnce() {
  try {
    ...
    document.querySelectorAll('iframe[src]').forEach(ifr => {
      ...
      // ===== NUEVO: Detectar CUALQUIER iframe de otro dominio =====
      try {
        const iframeOrigin = new URL(s).origin;
        const pageOrigin = location.origin;
        
        // Si no es un proveedor conocido Y es de un dominio diferente,
        // tratar el iframe en sí como un "vídeo" potencial.
        if (iframeOrigin !== pageOrigin) {
          chrome.runtime.sendMessage({ 
            type:'videoFound', 
            url:s, // La URL es el src del iframe
            via:'embed-iframe', 
            frameUrl: location.href, 
            meta: { provider:'iframe' }
          });
        }
      } catch (e) { /* ignorar URLs inválidas (ej: 'about:blank') o relativas */ }
      // ==========================================================
    });
...
// NOTA: Se han eliminado los listeners de 'dbg.dumpDOM' 
// y el volcado automático de 2 segundos.
// El volcado de DOM ahora se maneja completamente desde popup.js