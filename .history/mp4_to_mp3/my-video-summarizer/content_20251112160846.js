...
function wireVideo(v) {
  ...
  const send = (withThumb=false) => {
    ...
    // --- CAMBIO CLAVE ---
    // Enviamos también el frameUrl para que background.js sepa
    // exactamente qué vídeo actualizar.
    chrome.runtime.sendMessage({ 
      type: 'videoState', 
      url: state.url, 
      frameUrl: state.frameUrl, // <-- AÑADIDO
      meta: meta 
    }).catch(()=>{});
  };
...
function scanOnce() {
  try {
    ...
    document.querySelectorAll('video').forEach(v => {
      ...
      if (u) chrome.runtime.sendMessage({ type:'videoFound', url:String(u), via:'video-tag', frameUrl: location.href, meta }).catch(()=>{});
      
      v.querySelectorAll('source').forEach(s => {
        if (s.src) chrome.runtime.sendMessage({ type:'videoFound', url:String(s.src), via:'source-tag', frameUrl: location.href, meta }).catch(()=>{});
      });
    });
