...
chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
  try{
    ...
    // MENSAJE PARA VOLCAR EL DOM (desde content.js)
    // ### INICIO DE LA MODIFICACIÓN ###
    if(msg?.type==='dbg.logDOM'){
      try {
        const htmlString = msg.dom || '';
        if (!htmlString) {
          devlog('DOM_PARSE', 'DOM vacío recibido', { frameUrl: msg.frameUrl || '?' });
          sendResponse({ok: false, error: 'DOM vacío'});
          return true;
        }

        // 1. Usamos DOMParser para convertir el string HTML en un documento
        const parser = new DOMParser();
        const doc = parser.parseFromString(htmlString, 'text/html');

        // 2. Buscamos los elementos de vídeo, iframes, object y embed
        const mediaElements = doc.querySelectorAll('video, iframe, object, embed');
        
        // 3. Extraemos su HTML (outerHTML) para el log
        const mediaHtmlList = [];
        mediaElements.forEach(el => {
          mediaHtmlList.push(el.outerHTML);
        });

        // 4. Registramos solo esta información
        devlog('DOM_VIDEOS', 
          `Encontrados ${mediaHtmlList.length} elementos <video/iframe/obj/embed> en ${msg.frameUrl || '?'}`, 
          { 
            count: mediaHtmlList.length,
            elements: mediaHtmlList, // Lista de strings HTML de cada etiqueta
            frameUrl: msg.frameUrl || '?'
          }
        );
        
        sendResponse({ok:true, count: mediaHtmlList.length});

      } catch (e) {
        // Registrar cualquier error durante el parseo
        devlog('DOM_PARSE_ERR', `Error analizando el DOM de ${msg.frameUrl || '?'}`, { error: e?.message || String(e), frameUrl: msg.frameUrl });
