...
const dbgDomBtn     = document.getElementById('dbgDom'); // Botón para volcar el DOM
...
// --- Lógica para el nuevo botón "Volcar DOM" ---
if (dbgDomBtn) {
  dbgDomBtn.onclick = async () => {
    const tab = await activeTab();
    if (!tab) return;
    dbgDomBtn.disabled = true;
    dbgDomBtn.textContent = 'Volcando...';
    // Pide al content.js de esa pestaña que envíe su DOM
    try {
      // El content.js escuchará este mensaje
      await chrome.tabs.sendMessage(tab.id, { type: 'dbg.dumpDOM' });
      // (El resultado aparecerá en el logger_server.js)
      alert('Petición de DOM enviada. Revisa tu terminal de logger_server.js');
    } catch (e) {
      alert('Error al contactar la pestaña: ' + (e.message || e));
    }
    dbgDomBtn.disabled = false;
    dbgDomBtn.textContent = 'Volcar DOM';
  };
}
