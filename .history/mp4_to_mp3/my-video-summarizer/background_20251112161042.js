...
const DEVLOG_ENABLED = false; // Desactivado para evitar errores de red
...
function pushVideoToDoc(tabId,docKey,entry){
  ...
  if(bucket.index.has(key)){ 
    const i=bucket.index.get(key); 
    const prev=bucket.list[i]; 
    // --- CORRECCIÓN (Thumbnail fix) ---
    // Combinamos 'meta' para no perder la miniatura si un mensaje llega sin ella
    bucket.list[i]={...prev, ...entry, ts:prev.ts||Date.now(), meta:{...(prev.meta||{}), ...(entry.meta||{})}}; 
    ...
    if(msg?.type==='getVideos'){
      (async ()=>{
        ...
        for(const e of bucket.list){
          ...
          // --- CORRECCIÓN (Filtro) ---
          // Permitimos cualquier vídeo de http, https, o blob.
