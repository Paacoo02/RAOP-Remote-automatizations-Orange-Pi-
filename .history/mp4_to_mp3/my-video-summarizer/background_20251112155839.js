...
const DEVLOG_ENABLED = false;
const DEVLOG_URL = 'http://172.17.0.1:8765/log';
...
async function devlog(tag, msg, extra) {
  ...
  } catch {/* no romper el SW si falla */}
}
...
function pushVideoToDoc(tabId,docKey,entry){
  ...
  if(bucket.index.has(key)){ const i=bucket.index.get(key); const prev=bucket.list[i]; bucket.list[i]={...prev,...entry,ts:prev.ts||Date.now()}; devlog('VIDEO','update',{tabId,docKey,key,url:entry.url}); }
  else { bucket.list.push({...entry,ts:Date.now()}); bucket.index.set(key,bucket.list.length-1); devlog('VIDEO','new',{tabId,docKey,key,url:entry.url,frameUrl:entry.frameUrl}); }
...
chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
  try{
    ...
    if(msg?.type==='getVideos'){
      (async ()=>{
        ...
        for(const e of bucket.list){
          ...
          // --- ¡CAMBIO CLAVE! ---
          // La lógica anterior (originOK) era demasiado estricta (solo mismo dominio).
          // Ahora permitimos cualquier vídeo de http, https, o blob.
          const originOK = videoOrigin.startsWith('http') || videoOrigin === 'blob-data://';
          ...
          reasons.push(originOK ? 'originOK=true' : 'originOK=false (filtered)');
    ...
    // --- Receptor para el botón "Descargar" ---
    if (msg?.type === 'downloadViaApi') {
      (async () => {
        ...
        // Asumimos que tienes un servidor local en el puerto 8765
        // y que el endpoint para descargar es /download
        const API_ENDPOINT = 'http://127.0.0.1:8765/download';
        log('DOWNLOAD', 'Enviando a la API...', { url });
        ...
        try {
          ...
          log('DOWNLOAD', 'Respuesta de la API', result);
          sendResponse({ ok: true, data: result });

        } catch (e) {
          const err = e?.message || String(e);
          log('DOWNLOAD', 'Error al contactar API', { error: err });
