...
// ===================================================================
// VUELCO DE DOM AUTOMÁTICO (NUEVO)
// ===================================================================
try {
  // Esperar 2 segundos después de 'document_idle' para dar tiempo
  // a que los reproductores de JS (como el de Marianistas) se rendericen.
  setTimeout(() => {
    console.log('VID_EXT (content.js): Volcando DOM automáticamente...');
    const dom = document.documentElement.outerHTML;
    
    // Enviar el DOM al background.js, que lo reenviará al logger
    chrome.runtime.sendMessage({ 
      type: 'dbg.logDOM', 
      dom: dom,
      frameUrl: location.href,
      via: 'auto-dump-2s' // Identificador
    }).catch((e) => console.warn('VID_EXT: Fallo al enviar DOM automático', e));
  
  }, 2000); // 2000ms = 2 segundos

} catch (e) {
  console.error('VID_EXT (content.js): Fallo en el volcado automático de DOM', e);
}


// ===================================================================
// BLOQUE AÑADIDO: Oyente para peticiones desde el POPUP (se mantiene)
// ===================================================================
try {
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    // Asegurarse de que no es un mensaje de 'window.postMessage' (distinguir de __VIDHOOK__)
    if (msg.__VIDHOOK__) return false; 

    // Petición desde popup.js para volcar el DOM (manual)
    if (msg?.type === 'dbg.dumpDOM') {
      console.log('VID_EXT (content.js): Recibida petición de volcado de DOM (manual)');
      const dom = document.documentElement.outerHTML;
      ...
      chrome.runtime.sendMessage({ 
        ...
        dom: dom,
        frameUrl: location.href,
        via: 'manual-dump' // Identificador
      }).catch((e) => console.warn('VID_EXT: Fallo al enviar DOM al background', e));
      
      sendResponse({ok: true, sent: dom.length});
      return true; // Indica que responderemos (aunque ya lo hicimos)
    }
    
    // Mensaje no reconocido por este listener
    return false;
  });
} catch (e) {
  console.error('VID_EXT (content.js): Fallo al añadir el listener de mensajes del popup', e);
}