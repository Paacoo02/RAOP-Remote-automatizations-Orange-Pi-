...
# NOTA(macOS): :0 suele ser el display de XQuartz. :99 puede fallar si Xvfb no está.
DISPLAY="${DISPLAY:-:0}"
...
# Esta ruta se sobrescribirá en playwright_ensure() con la correcta para el S.O.
CHROMIUM_PATH="/ms-playwright/chromium-latest/chrome-linux/chrome"
...
wait_for_port() {
  ...
  for _ in $(seq 1 "${tries}"); do 
    # CAMBIO(macOS): 'ss' no existe en macOS. Usamos 'netstat'.
    # La sintaxis de netstat en macOS (BSD) es distinta a la de Linux.
    # Buscamos ".<puerto> " o ".<puerto>." en estado LISTEN.
...
playwright_ensure() {
  ...
  # CAMBIO(macOS): Detectar S.O. y asignar la ruta correcta a CHROMIUM_PATH
  if [[ "$(uname)" == "Darwin" ]]; then
    CHROMIUM_PATH="${FOUND_DIR}/chrome-mac/Chromium.app/Contents/MacOS/Chromium"
  else
    CHROMIUM_PATH="${FOUND_DIR}/chrome-linux/chrome"
  fi
  
  [ -x "${CHROMIUM_PATH}" ] || fail "No se encontró el ejecutable Chromium en ${CHROMIUM_PATH}"
  
  for alias in chromium-latest chromium-1193 chromium-1194; do
    ln -sfn "${FOUND_DIR}" "${BASE}/${alias}"
  done
  echo "✅ Chromium listo en ${CHROMIUM_PATH}"
}
...
start_ssh() {
  ...
  # CAMBIO(macOS): Usar launchctl en lugar de 'service'
  if [[ "$(uname)" == "Darwin" ]]; then
    echo "ℹ️  (macOS) Intentando iniciar SSH con launchctl (puede pedir sudo)..."
    # Esto puede pedir contraseña
    sudo launchctl load -w /System/Library/LaunchDaemons/ssh.plist >/tmp/ssh_start.log 2>&1 || true
  else
    service ssh start >/tmp/ssh_start.log 2>&1 || true
  fi
  
  pgrep -x sshd >/dev/null && echo "✅ SSH activo en puerto 22" || echo "⚠️ SSH no pudo iniciarse (revisar logs o 'Preferencias del Sistema > Compartir')"
}

# =================== VNC / noVNC ===================
start_vnc_stack() {
  echo "🖥️  Iniciando Xvfb en ${DISPLAY} (${XVFB_RESOLUTION})"
  # NOTA(macOS): Esto asume que tienes Xvfb (parte de XQuartz).
  # Si DISPLAY=:0, esto fallará si XQuartz ya está corriendo en :0.
  # El script original forzaba :99. Lo hemos cambiado a :0 por defecto.
...
launch_chromium_with_extension() {
  ...
  # (macOS): xset puede existir si XQuartz está instalado
  ...
  # CAMBIO(macOS): No buscar 'chrome-linux/chrome', sino el argumento 'user-data-dir'
  # que es único para esta instancia.
  if ! pgrep -f "user-data-dir=/tmp/chrome-profile" >/dev/null; then
    ...
    sleep 3
    # CAMBIO(macOS): Búsqueda por flag también aquí
    if pgrep -f "user-data-dir=/tmp/chrome-profile-fallback" >/dev/null; then
...
if [[ "${HEADLESS}" == "false" ]]; then
  # NOTA(macOS): Esto asume que tienes XQuartz y los componentes (Xvfb, fluxbox, x11vnc)
  # instalados y en tu PATH. Si no, fallará.
  # Si quieres que se lance en tu escritorio normal, es probable que tengas que
  # comentar la llamada a start_vnc_stack y asegurarte que DISPLAY=:0
  export DISPLAY
  start_vnc_stack
else
