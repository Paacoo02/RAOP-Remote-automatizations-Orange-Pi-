...
const dbgDumpDOMBtn = document.getElementById('dbgDumpDOM'); // ¡NUEVO BOTÓN!
...
// *** ¡LÓGICA ACTUALIZADA PARA VOLCAR DOM! ***
// Esta es la función que se inyectará en CADA frame
function _dom_filter_func() {
  const tags = document.querySelectorAll('video, iframe, source, meta[property="og:video"]');
  if (!tags || tags.length === 0) {
    return '(No <video>, <iframe>, <source>, or og:video tags found in this frame)';
  }
  return Array.from(tags).map((el, i) => {
    return `  [${i + 1}/${tags.length}] ${el.tagName}:\n  ${el.outerHTML}`;
  }).join('\n');
}

// Lógica del botón "Volcar DOM"
dbgDumpDOMBtn.onclick = async () => {
  dbgDumpDOMBtn.disabled = true;
  dbgDumpDOMBtn.textContent = 'Volcando...';
  try {
    const tab = await activeTab();
    if (!tab) {
    ...
    // Inyectar el script en TODOS los frames de la pestaña
    const results = await chrome.scripting.executeScript({
      target: { tabId: tab.id, allFrames: true },
      func: _dom_filter_func,
    });

    // Enviar los resultados (un array) al background.js
    await ask({ 
      type: 'dbg.logFilteredDOM', 
      results: results.map(r => ({
        frameId: r.frameId,
        frameUrl: r.documentId, // Usamos documentId como aproximación
        html: r.result || '(Script returned null)'
      }))
    });
    alert('DOM Filtrado enviado al logger de la terminal.');
  } catch (e) {
    console.error('Fallo al volcar DOM', e);
    alert('Error al volcar DOM: ' + (e?.message || String(e)));
  } finally {
    dbgDumpDOMBtn.disabled = false;
    dbgDumpDOMBtn.textContent = 'Volcar DOM';
  }
};
