// drive_auto.js — Abre Colab, monta Drive, espera link/JSON y (solo si ok=true) descarga/borra video.mp3
'use strict';
...
/* === Espera de enlace trycloudflare o JSON en la celda #3 === */
async function waitForCloudflareLinkOrJsonInCell(page, idx = 2, { timeoutMs = 300000, pollMs = 300 } = {}) {
  console.log(`👂 Esperando enlace trycloudflare.com o JSON en celda #${idx}…`);
  const handle = await page.waitForFunction?.(
    (i) => {
      const cell = document.querySelectorAll('.cell.code')[i];
      if (!cell) return false;

      // 1) Link trycloudflare en outputs
      const deep = [];
      ...
      // 2) Último texto de salida → intentar parsear JSON
      const outSelectors = [
      ...
      const lastLine = last.split(/\r?\n/).map(s => s.trim()).filter(Boolean).pop() || last;
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  try {
    const runAnyway = colabPage.locator?.('colab-dialog button:has-text("Run anyway")')?.first?.();
    if (runAnyway && await runAnyway.count?.()) { await runAnyway.click?.(); await sleep(200); }
  } catch {}
  try {
    const welcomeClose = colabPage.locator?.('colab-dialog[class*="welcome-dialog"] #close-icon')?.first?.();
    if (welcomeClose && await welcomeClose.count?.()) { await welcomeClose.click?.(); await sleep(150); }
  } catch {}
  ...
  console.log('3️⃣ Ejecutando Celda 3 (conversión + JSON)…');
  await runCellByIndex(colabPage, 2, false);
  const outcome = await waitForCloudflareLinkOrJsonInCell(colabPage, 2, { timeoutMs: 300000 });
  if (outcome?.kind === 'true') {
    ...
    for (let attempt = 1; attempt <= MAX_DOWNLOAD_ATTEMPTS; attempt++) {
      try {
        ...
        // FORZAR RECARGA: Clave para que Drive vea el archivo nuevo
        if (attempt > 1) {
          ...
          // Espera a que la UI esté lista *después* de recargar
        ...
        lastDownloadError = null; // Limpiamos el error, éxito
        break; // Salimos del bucle si tiene éxito
      
      } catch (err) {
        console.warn(`⚠️ [Intento ${attempt}/${MAX_DOWNLOAD_ATTEMPTS}] Falló método por menú:`, err.message);
        lastDownloadError = err; // Guardamos el último error
    ...
    // Si después de todos los intentos sigue habiendo un error, lo lanzamos
