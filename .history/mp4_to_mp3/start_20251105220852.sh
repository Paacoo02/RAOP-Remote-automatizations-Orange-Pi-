...
# ======================================================
# 🚀 ¡ARREGLO LÓGICO!
# Esta función ahora solo verifica lo que el Dockerfile ya hizo.
# ======================================================
playwright_ensure() {
  echo "🔎 Verificando binarios de Playwright en ${PLAYWRIGHT_BROWSERS_PATH}..."
  
  # El Dockerfile YA instaló la versión correcta de Chromium (para v1.47.2)
  # Solo necesitamos comprobar que *un* navegador (cualquier versión) exista.
  ...
  echo "⚠️  No se encontraron binarios de Chromium."
  echo "Esto es un error en el build del Dockerfile, pero intentaremos instalarlo ahora..."
  
  # Como plan B, instalamos la versión definida en el Dockerfile (usará el package.json)
  npx playwright install --with-deps chromium
  
  if ! ls "${PLAYWRIGHT_BROWSERS_PATH}"/chromium-*/chrome-linux/chrome >/dev/null 2>&1; then
     echo "❌ FALLO: No se pudo instalar Chromium en runtime."
     return 1
  fi
}
