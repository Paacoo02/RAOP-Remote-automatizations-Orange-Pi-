...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  if (outcome?.kind === 'true') {
    ...
    // Nuevo método principal: descarga por menú contextual
    let dlInfo = null;
    try {
      const { downloadAndTrashFileViaMenu } = require('./auto_log_in.js');
      dlInfo = await downloadAndTrashFileViaMenu(
        videosTab,
        'video.mp3',
        { destDir: (typeof os?.tmpdir === 'function' ? os.tmpdir() : '/tmp'), timeoutMs: 120000 }
      );
      console.log('✅ Descarga completada por menú:', dlInfo.path);
    } catch (err) {
      ...
      try {
        ...
        console.log('✅ Descarga completada por método antiguo:', dlInfo.path);
      } catch (e2) {
