...
# ======================================================
# "DIRECTO AL GRANO": Solo verificamos el build.
# ======================================================
playwright_ensure() {
  echo "🔎 Verificando que 'chromium-1193' esté instalado (desde el build)..."
  if ls -d "${PLAYWRIGHT_BROWSERS_PATH}/chromium-1193" >/dev/null 2>&1; then
     echo "✅ 'chromium-1193' encontrado. ¡Todo listo!"
     return 0
  fi
  
  echo " "
  echo "❌ FALLO CRÍTICO DE BUILD: 'chromium-1193' no existe."
  echo "   El Dockerfile no se construyó correctamente (probablemente 'npm install' falló)."
  echo "   Contenido de ${PLAYWRIGHT_BROWSERS_PATH}:"
  ls -la "${PLAYWRIGHT_BROWSERS_PATH}"
  echo " "
  # Detenemos el script si el navegador no está.
  exit 1
}
...
playwright_ensure # Esto ahora fallará si el build no fue perfecto

# ====== Lanzamiento de tu app ======
PYTHON_SCRIPT="/app/main.py"
VENV_PATH="/app/venv"

if [[ -f "${PYTHON_SCRIPT}" ]] && [[ -f "${VNH_PATH}/bin/activate" ]]; then
  ...
  if [[ "${HEADLESS}" == "false" ]]; then export DISPLAY; fi
  # Ahora esto SÍ debería reportar v1.40.0
