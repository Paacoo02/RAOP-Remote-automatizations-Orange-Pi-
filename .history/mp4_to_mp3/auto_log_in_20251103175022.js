...
/* === ESPERA ESTRICTA: fila real <tr role="row" data-id> === */
/* === ESPERA ESTRICTA ROBUSTA: detectar <tr role="row" data-id> con el nombre === */
async function waitForFileRowByDataIdOrName(page, fileName, timeoutMs = 5 * 60 * 1000) {
  ...
  // Para re-render virtualizado
  async function nudgeGrid() {
    try {
      // intenta mover el scroll del grid principal
      const grid = page.locator('div[role="main"] [role="grid"]').first();
      if (await grid.count()) {
  ...
  async function findRow() {
    return await page.evaluate((name) => {
      // 1) Candidatas: todas las filas reales renderizadas
      const rows = Array.from(document.querySelectorAll('tr[role="row"][data-id]'));
      for (const tr of rows) {
        // a) Nombre visible en <strong.DNoYtb> o en celda con data-tooltip / aria-label
        const strong = tr.querySelector('strong.DNoYtb');
        const nameCell =
          strong?.textContent?.trim() ||
          tr.querySelector('.i92Sbe[data-id]')?.textContent?.trim() ||
          tr.querySelector('[data-tooltip]')?.getAttribute('data-tooltip') ||
          tr.getAttribute('aria-label') || '';

        if (!nameCell) continue;

        // b) Coincide el nombre
        if (!nameCell.includes(name)) continue;

        // c) Evitar filas de "subiendo…" sólo si el spinner está dentro de la propia fila
        const rowHasSpinner =
          tr.querySelector('[role="progressbar"], [aria-busy="true"], .upload-progress, [aria-live="polite"]');
        if (rowHasSpinner) continue; // todavía no es la fila final lista

        const id = tr.getAttribute('data-id');
        if (id) {
          return { id, html: tr.outerHTML };
        }
      }
      return null;
    }, fileName);
  ...
  // Bucle con nudges y con 1 re-render duro (Recent -> back)
  let didHardRefresh = false;

  while (Date.now() - t0 < timeoutMs) {
    const found = await findRow().catch(() => null);
    if (found?.id) {
      const row = page.locator(`tr[role="row"][data-id="${found.id}"]`).first();
      await row.waitFor({ state: 'visible', timeout: 4000 }).catch(()=>{});
      return { fileId: found.id, row };
    }
    ...
    // si no aparece tras ~7s, fuerza re-render y reintenta sólo una vez
    if (!didHardRefresh && Date.now() - t0 > 7000) {
      didHardRefresh = true;
      try {
        await openRecentAndComeBack(page);
      } catch {}
    }
  }

  throw new Error(`Timeout esperando <tr role="row" data-id> con "${fileName}".`);
}
...
async function uploadFileToSpecificFolderUI(filePath) {
  ...
  try {
    ...
    console.log('🏁 Final (fila real):', fastResult);
    return fastResult;
