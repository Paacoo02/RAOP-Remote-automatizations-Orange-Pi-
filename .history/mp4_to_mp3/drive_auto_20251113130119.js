...
async function handleOAuthPopupByEmailOrForm(p) {
  ...
  try {
    ...
    if (emailBox && await emailBox.count?.()) {
      ...
      await emailBox.type?.(EMAIL, { delay: 25 }).catch(()=>{}); // <-- Optimización: delay 40 -> 25
    ...
    if (passBox && await passBox.count?.()) {
      ...
      await passBox.type?.(PASS, { delay: 25 }).catch(()=>{}); // <-- Optimización: delay 40 -> 25
...
async function confirmYesOkDialogs(page) {
  ...
  await sleep(100); // <-- Optimización: sleep 300 -> 100
...
async function waitForCellToFinish(page, idx = 0, { timeoutMs = 300000, pollMs = 250 } = {}) {
  ...
  await new Promise(r => setTimeout(r, 150)); // <-- Optimización: sleep 400 -> 150
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  // Dialogs
  try { const runAnyway = colabPage.locator?.('colab-dialog button:has-text("Run anyway")')?.first?.(); if (runAnyway && await runAnyway.count?.()) { await runAnyway.click?.(); await sleep(200); } } catch {} // <-- Optimización: 600 -> 200
  try { const welcomeClose = colabPage.locator?.('colab-dialog[class*="welcome-dialog"] #close-icon')?.first?.(); if (welcomeClose && await welcomeClose.count?.()) { await welcomeClose.click?.(); await sleep(150); } } catch {} // <-- Optimización: 300 -> 150
  ...
  try {
    ...
    await runBtn0?.click?.();
    await sleep(400); // <-- Optimización: 1200 -> 400
  } catch {}
  ...
  await sleep(1000); // <-- Optimización: 3500 -> 1000
