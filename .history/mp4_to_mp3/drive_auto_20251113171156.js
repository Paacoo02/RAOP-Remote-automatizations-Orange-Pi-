...
async function waitForCellToFinish(page, idx = 0, { timeoutMs = 300000, pollMs = 250 } = {}) {
  ...
  const handle = await page.waitForFunction?.(
    (i) => {
      ...
      if (running) {
        // console.log(`[Debug-Page] Celda ${i} SIGUE corriendo.`); // Log demasiado ruidoso
        return false;
      }
      ...
      if (hasCheck || hasError) {
         console.log(`[Debug-Page] Celda #${i} TERMINADA. Check: ${hasCheck}, Error: ${hasError}`);
         return { ok: !!hasCheck, err: !!hasError };
      }
      
      // console.log(`[Debug-Page] Celda #${i} ni corriendo, ni check, ni error.`); // Log demasiado ruidoso
      return false;
    },
    idx,
...
async function waitForMountedDriveInCell(page, idx = 1, { timeoutMs = 180000, pollMs = 300 } = {}) {
  ...
  const h = await page.waitForFunction?.(
    (i) => {
      ...
      // Buscar el texto en *todos* los nodos recolectados
      const allText = nodes.map(n => (n.textContent || '')).join('\n').toLowerCase();
      
      if (allText.includes('mounted at /content/drive') || allText.includes('montado en /content/drive')) {
        console.log('[Debug-Page] Texto de montaje "Mounted at..." ENCONTRADO.');
        return true;
      }
      return false;
    },
    ...
    console.warn(`[Debug] waitForMountedDriveInCell falló o expiró. ${err.message}`);
    return null; // No fallar, dejar que el 'tick' gane la carrera si es necesario
  });
  
  if (!h) {
      console.warn('⚠️ No vimos el texto "Mounted at...", confiamos en el tick ✓ si llega.');
  } else {
      console.log('✅ Texto "Mounted at..." detectado.');
  }
  return !!h;
}
...
async function runCellByIndex(page, idx, waitTick = false) {
  ...
  console.log(`[Debug] Clickeando Run Button de celda #${idx}...`);
  await runBtn?.click?.();
  console.log(`[Debug] Click enviado a celda #${idx}.`);
  
  if (waitTick) {
    console.log(`[Debug] Celda #${idx} debe esperar el tick.`);
    ...
    (i) => {
      ...
      if (texts.length) {
        ...
        for (const t of texts) {
            ...
            if (lastLine === 'True') {
                console.log('[Debug-Page] "True" ENCONTRADO.');
                return { kind: 'true', value: true };
            }
        }
      }
      // console.log('[Debug-Page] "True" aún no encontrado...'); // Log demasiado ruidoso
        ...
        if (lastLine.startsWith('{') && lastLine.endsWith('}')) {
          try {
            ...
            if (typeof parsed.ok === 'boolean') {
              console.log('[Debug-Page] JSON extraído:', lastLine);
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  // ** CORREGIDO: Ambas funciones ahora usan Shadow DOM **
  await Promise.race([
