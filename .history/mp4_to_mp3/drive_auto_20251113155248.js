...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  // Dialogs
  // Usamos los delays del código de referencia que me pasaste
  ...
  try {
    ...
    await runBtn0?.click?.();
    await sleep(1200); // <-- Restaurado a 1.2s
  } catch {}
  ...
  console.log('⏳ Espera breve (3.5s) para que aparezca el diálogo...'); 
  await sleep(3500); // <-- Restaurado a 3.5s (valor original)

  // Intentamos enfocar el botón. Si falla (focused=false), no importa.
  ...
  // Pulsamos 'Enter' INCONDICIONALMENTE
