...
/* === CORREGIDO: Espera JSON (recursivo) en la celda #2 === */
async function waitForJsonOutputInCell(page, idx = 2, { timeoutMs = 300000, pollMs = 300 } = {}) {
  ...
  const handle = await page.waitForFunction?.(
    (i) => {
      ...
      // 1. Colector recursivo que SÍ funciona (explora Shadow DOM)
      const nodes = [];
      (function collect(root, acc) {
        if (!root) return;
        acc.push(root);
        // Hijos directos de Shadow DOM
        if (root.shadowRoot) {
            // Recolectar hijos del shadow root
            const shadowChildren = root.shadowRoot.querySelectorAll('*');
            for (const el of shadowChildren) {
                collect(el, acc); // Recursión
            }
        }
        // Hijos normales
      ...
      // 2. Iterar todos los nodos, al revés (del final al principio)
      for (let j = nodes.length - 1; j >= 0; j--) {
        const node = nodes[j];
        if (!node) continue;
        
        // Usar textContent, queremos el texto "crudo"
        const text = (node.textContent || '').trim();
        if (!text) continue;

        // El texto puede tener múltiples líneas
        const lines = text.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
        if (!lines.length) continue;

        // Coger la última línea de este nodo
        const lastLine = lines[lines.length - 1];

        // Comprobación robusta de JSON
        if (lastLine.startsWith('{') && lastLine.endsWith('}')) {
          try {
            const parsed = JSON.parse(lastLine);
            // Validar que es el JSON que esperamos (debe tener 'ok')
            if (typeof parsed.ok === 'boolean') {
              return parsed; // ¡Encontrado!
            }
          } catch (e) {
            // Falso positivo (texto que parece JSON pero no lo es)
            // Continuar buscando...
          }
        }
      }
