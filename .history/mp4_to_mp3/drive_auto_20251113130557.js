...
// ... (Las funciones clickByTextAcrossFrames, ensureVisibleAndFocused, stepThroughConsent, 
// waitAndFocusConnectButton, waitForOAuthCascade, handleOAuthPopupByEmailOrForm, 
// openRuntimeMenu, clickRuntimeRestartLike, confirmYesOkDialogs, 
// restartRuntimeFlexible, y waitForCellToFinish permanecen EXACTAMENTE IGUALES) ...

// ... [PEGAR AQUÍ LAS FUNCIONES DESDE clickByTextAcrossFrames HASTA waitForCellToFinish] ...
...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  await colabPage.bringToFront?.();
  console.log('🕰️ Esperando finalización Celda 2 (montaje)...');

  // -----------------------------------------------------------------
  //  INICIO: LÓGICA MODIFICADA PARA CELDA 2
  // -----------------------------------------------------------------
  
  // Ya no usamos waitForMountedDriveInCell.
  // 1. Esperamos a que la celda termine (tick).
  // 2. Leemos el JSON que ha impreso.
  try {
