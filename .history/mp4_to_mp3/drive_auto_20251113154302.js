...
async function drive_auto({ context: injectedContext, drivePage: injectedDrivePage } = {}) {
  ...
  try {
    ...
    console.warn(`⚠️ Fallo al ejecutar la celda 2 (idx 1): ${e.message}`);
  }
  
  console.log('⏳ Esperando ~5 segundos a que aparezca el diálogo de montaje...'); 
  // Aumentamos el sleep a 5 segundos (de 2s) para dar tiempo de sobra a que aparezca el diálogo.
  // El 'sleep(3500)' del código de referencia original estaba ahí por algo.
  await sleep(5000); 

  // (Opcional) Intentamos enfocar el botón. Esto es "polite".
  // Usamos un timeout corto (5s) porque el diálogo ya debería estar visible.
  const focused = await waitAndFocusConnectButton(colabPage, 5000).catch(() => false); 
  if (!focused) {
      console.log('ℹ️ No se pudo enfocar el botón "Conectar" (quizás ya montado o no visible), pulsando ENTER igualmente.');
  } else {
      console.log('✅ Diálogo "Conectar con Google Drive" detectado.');
  }

  // Pulsamos 'Enter' INCONDICIONALMENTE, como en el flujo original.
  console.log('↩️ Pulsando ENTER para aceptar el diálogo "Conectar".');
  ...
  // Manejamos los popups de OAuth INCONDICIONALMENTE.
  // Si el 'Enter' anterior no hizo nada (porque ya estaba montado), esta función
  // simplemente esperará su 'idleMs' (2s) y no encontrará popups, terminando limpiamente.
  console.log('🔐 Esperando cascada de popups de OAuth (si la hay)...');
