...
# ==========================================
# DIRECTOR (crawler + prefilter + thumbnail-probe + batched matching + GPU debug)
# Servidor universal requerido:
#   - GET  /health     -> {"cuda_available": bool, "gpu_name": "...", "upload_directory": "..."}
#   - POST /upload     -> files={"file": (...)}  => {"file_path": "..."}
#   - POST /execute    -> json={"script_content": "...", "params":[...]} => JSON del script
# ==========================================

import cv2
import numpy as np
import requests, re, os, sys, time, json, queue, threading
...
TOPK_CANDIDATES          = 260  # tras heurística html
TOPM_AFTER_PROBE         = 90   # tras probe de miniaturas
ASPECT_TOL_BASE          = 0.35 # ±35% base
ASPECT_TOL_MAX           = 0.70 # hasta ±70%
ASPECT_REJECT_FLOOR      = 0.20 # mínimo ±20%
HIST_THR_PROBE           = 0.12 # hist bajo para probe rápido
HIST_TOL_SKIP_ASPECT     = 0.25 # si hist>=0.25 no descartes por aspecto
EDGE_MIN_PROBE           = 450  # textura mínima en probe (Canny sum)
LAP_MIN_PROBE            = 3.5  # textura mínima alternativa (Laplacian var)
SCALE_MIN                = 0.25
SCALE_MAX                = 1.50
N_SCALES                 = 12
EARLY_STOP_THRESHOLD     = 0.92

# silenciar warnings de requests SSL y opencv
...
# Metadatos de imágenes: { url: {"w":int|None,"h":int|None,"ext":".jpg",".png",...,"from":page_url} }
image_meta = {}
...
# ----------------------------
# Silenciar libpng durante imdecode + limpieza eXIf
# ----------------------------
@contextmanager
def suppress_stderr():
...
def strip_png_exif_chunks(data: bytes) -> bytes:
    """Elimina todos los chunks eXIf de un PNG (si los hubiera)."""
...
def decode_small_and_full(data_bytes: bytes):
    b = data_bytes
...
def rank_and_cut_candidates(crop_w, crop_h):
    ...
    def ext_weight(ext):
        if ext in (".jpg", ".jpeg", ".png", ".webp"): return 0.0
    ...
    for url, meta in image_meta.items():
        ...
        if w and h:
            ar_img = (w / max(1,h))
            ar_diff = abs(ar_img - ar_crop)  # solo razón de aspecto, tamaño libre
        else:
    ...
    top = [u for _, u in ranked[:min(TOPK_CANDIDATES, len(ranked))]]
    return top
...
def probe_one(url, crop_gray, ar_crop, tol):
    ...
    small, _ = decode_small_and_full(b)
    if small is None:
    ...
    # Aspecto con tolerancia adaptativa (NO exige mismo tamaño)
    aspect_ok = (1 - tol <= ar_ratio <= 1 + tol) or (hist >= HIST_TOL_SKIP_ASPECT)

    # textura mínima
    texture_ok = (edg >= EDGE_MIN_PROBE) or (lap >= LAP_MIN_PROBE)
    ...
    # score preferente (menor mejor): |ar_diff| + (1-hist) + pequeña penalización si poca textura
...
def probe_candidates(candidates, crop_gray, ar_crop):
    ...
    # Debug: distribución de rechazos
    if rejects:
        cnt = Counter([str(r.get("why","")) for r in rejects])
        top = ", ".join([f"{k}:{v}" for k,v in (cnt.most_common(8))])
        print(f"    [probe] rechazos: {top}", file=sys.stderr)
    ...
    # TOP-M al matching
    keep_urls = [k[1]["url"] for k in keep[:min(TOPM_AFTER_PROBE, len(keep))]]
    print(f"🎯 Probe final: {len(keep_urls)}/{len(candidates)} pasan a matching (TOP-M={TOPM_AFTER_PROBE}).", file=sys.stderr)

    # Debug: muestra 5 mejores
    if keep:
        sample_best = keep[:min(5, len(keep))]
        print("    [probe] mejores (pref, w, h, hist, edges, lap):", file=sys.stderr)
        for pref, info in sample_best:
            print(f"      {pref:.3f}  {info['w']}x{info['h']}  h:{info['hist']}  e:{info['edges']}  l:{info['lap']}  {info['url']}", file=sys.stderr)

    return keep_urls

# ----------------------------
# (Remoto) Script GPU por LOTES con DEBUG de tiempos/etapas
# ----------------------------
...
def url_to_cv2_fast_gray(url, headers=None):
    ...
    for _ in range(2):
        try:
            ...
            small = cv2.imdecode(npb, cv2.IMREAD_REDUCED_GRAYSCALE_4)
            full  = cv2.imdecode(npb, cv2.IMREAD_GRAYSCALE)
            return small, full, len(b), time.time()-t0
        except Exception:
            time.sleep(0.25)
...
def quick_reject(img_small_gray, crop_gray, aspect_tol=0.20, min_edge=900):
...
def score_one(full_url, crop_gray):
    t0 = time.time()
    small, full, nbytes, tdl = url_to_cv2_fast_gray(full_url)
    if small is None:
        return {"url": full_url, "score": -1.0, "scale": 1.0, "stage": "download_fail", "t_ms": int((time.time()-t0)*1000), "bytes": nbytes, "tdl_ms": int(tdl*1000)}
    rej, reason = quick_reject(small, crop_gray)
    if rej:
        return {"url": full_url, "score": -1.0, "scale": 1.0, "stage": f"rej:{reason}", "t_ms": int((time.time()-t0)*1000), "bytes": nbytes, "tdl_ms": int(tdl*1000)}
    if full is None:
        return {"url": full_url, "score": -1.0, "scale": 1.0, "stage": "decode_full_fail", "t_ms": int((time.time()-t0)*1000), "bytes": nbytes, "tdl_ms": int(tdl*1000)}
    ...
    if sc_e < 0.60:
        return {"url": full_url, "score": sc_e, "scale": sca_e, "stage": "edges_low", "t_ms": int((time.time()-t0)*1000), "t_edges_ms": int(t_edges*1000), "bytes": nbytes, "tdl_ms": int(tdl*1000)}

    t2 = time.time()
    sc_i, sca_i = run_match_int(full, crop_gray, scales)
    t_int = time.time() - t2
    out = {"url": full_url, "score": sc_i if sc_i>=sc_e else sc_e, "scale": sca_i if sc_i>=sc_e else sca_e,
           "stage": "int_done" if sc_i>=sc_e else "edges_done",
           "t_ms": int((time.time()-t0)*1000), "t_edges_ms": int(t_edges*1000), "t_int_ms": int(t_int*1000),
           "bytes": nbytes, "tdl_ms": int(tdl*1000)}
    return out

def pctl(arr, q):
    if not arr: return 0.0
    s=sorted(arr); k=int(round((len(s)-1)*q))
...
def main():
    ...
    tlist = [o.get("t_ms",0) for o in out]
    bytes_total = sum(int(o.get("bytes",0)) for o in out)
    stages = Counter([o.get("stage","n/a") for o in out])
    stats = {
        "count": len(out),
        "total_s": round(t_total,3),
        "avg_ms": round((sum(tlist)/len(tlist)) if tlist else 0.0, 1),
        "p95_ms": round(pctl(tlist, 0.95), 1),
        "bytes_mb": round(bytes_total/1_000_000.0, 3),
        "stages": dict(stages)
    }
    print(json.dumps({"items": out, "stats": stats}))
...
def server_health():
    try:
        r = session.get(f"{SERVER_BASE}/health", timeout=10)
        r.raise_for_status()
        return r.json()
    except Exception:
        return None
...
def main():
    ...
    if USE_REMOTE_EXEC:
        info = server_health()
        if info:
            print(f"🩺 /health -> cuda_available={info.get('cuda_available')} | gpu_name={info.get('gpu_name')} | upload_dir={info.get('upload_directory')}", file=sys.stderr)
        else:
            print("🩺 /health -> (sin respuesta)", file=sys.stderr)
    ...
    print(f"🔎 Prefilter sobre {total_found} imágenes...", file=sys.stderr)
    candidates = rank_and_cut_candidates(cw, ch)
    print(f"🎯 Prefiltro: {len(candidates)}/{total_found} candidatos tras heurística (TOP-K={TOPK_CANDIDATES}).", file=sys.stderr)
    ...
    if USE_REMOTE_EXEC and crop_remote:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS_MATCH) as ex:
            futs = []
            for bi, batch in enumerate(chunks(candidates2, BATCH_SIZE), 1):
                head = batch[0] if batch else "-"
                tail = batch[-1] if batch else "-"
                print(f"🚚 [GPU] Lote #{bi}: size={len(batch)} | first={head} | last={tail}", file=sys.stderr)
            ...
            for bi, fut in enumerate(as_completed(futs), 1):
                try:
                    resp = fut.result()
                    # Compatibilidad: array plano o dict con items+stats
                    if isinstance(resp, dict) and "items" in resp:
                        arr = resp.get("items", [])
                        stats = resp.get("stats", {})
                        # DEBUG GPU por lote
                        print(f"🧪 [GPU] Lote fin #{bi}: "
                              f"count={stats.get('count')}  total={stats.get('total_s')}s  "
                              f"avg={stats.get('avg_ms')}ms  p95={stats.get('p95_ms')}ms  "
                              f"bytes={stats.get('bytes_mb')}MB  stages={stats.get('stages')}", file=sys.stderr)
                    else:
                        arr = resp if isinstance(resp, list) else []
                        stats = None
                        print(f"🧪 [GPU] Lote fin #{bi}: (servidor legacy sin stats) items={len(arr)}", file=sys.stderr)
                except Exception as e:
                    print(f"💥 [GPU] Lote error: {type(e).__name__}: {e}", file=sys.stderr)
                    arr = []
                    stats = None
                ...
                for item in arr:
                    ...
                    if processed % 25 == 0 or processed == len(candidates2):
                        ...
                        print(f"    ... ⏳ {processed}/{len(candidates2)} ({dt:.1f}s, {rate:.1f} img/s) last_stage={stage}", file=sys.stderr)
                    if sc > best_score:
