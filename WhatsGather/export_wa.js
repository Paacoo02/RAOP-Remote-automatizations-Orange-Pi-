...
// 1) Escribimos el nombre NORMALIZADO (sin emojis, pero con "/") en el buscador.
// 2) En el DOM normalizamos igual (permitiendo "/") y comparamos.
async function openChatByTitle(page, chatTitle) {
  ...
  const normalizedForSearch =
    normalizeChatTitleForSearch(chatTitle) || chatTitle;

  console.log(
    `[FASE 2][DEBUG] Buscando chat "${chatTitle}" usando texto normalizado (sin emojis): "${normalizedForSearch}"`
  );
  ...
  await page.keyboard.type(normalizedForSearch, { delay: 40 });
  ...
  while (Date.now() - startedAt < maxWaitMs) {
    found = await page.evaluate((targetRaw) => {
      const normalize = (s) => {
        if (!s) return "";
        try {
          return s
      ...
      for (const row of rows) {
        ...
        if (
          normTitle === targetNorm ||
          normTitle.includes(targetNorm) ||
          targetNorm.includes(normTitle)
        ) {
          const clickable =
            row.querySelector('div[role="button"]') || row;
  ...
  if (!found) {
    throw new Error(
      `No se encontró el chat "${chatTitle}" en resultados de búsqueda usando clave "${normalizedForSearch}".`
    );
  }
...
async function processSingleChatOnPage(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (hasHistoryBanner || hasSyncBanner) {
        ...
        const reasons = [];
        if (hasHistoryBanner)
          reasons.push("phone-history-diff-post-export");
      ...
      console.log(
        `[${nowTs()}] [FASE 2] ⏱ Chat "${chat.title}" finalizado correctamente.`
      );
      return true;
    } catch (e) {
      lastError = e;
  ...
  console.log(
    `[${nowTs()}] [FASE 2] ⏱ Chat "${chat.title}" marcado como FALLIDO tras ${maxAttempts} intentos.`
  );
  return false;
}
