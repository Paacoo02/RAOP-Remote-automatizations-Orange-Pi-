...
const client = new Client({
  ...
  // Configuración de Puppeteer (el navegador "invisible" que usa)
  // para que funcione en servidores sin entorno gráfico
  puppeteer: {
    headless: true, // Siempre en modo invisible
    args: [
      '--no-sandbox',
      '--disable-setuid-sandbox',
      '--disable-dev-shm-usage',
      '--disable-gpu' // Clave para servidores
    ]
  }
});

// [1. GENERAR QR]
// Se dispara si no hay sesión guardada en .wa-session
client.on('qr', (qr) => {
  console.log('¡NUEVO QR RECIBIDO! Escanéalo con tu teléfono.');
  
  // "Inyecta" (dibuja) el QR directamente en tu terminal.
  // 
...
async function exportAllChatHistory() {
  ...
  // Procesamos los chats uno por uno
  for (const chat of chats) {
    ...
    // fetchMessages trae mensajes en lotes (de más nuevo a más antiguo).
    // Esta librería es mucho más eficiente que hacer scroll.
    while (keepFetching) {
...
async function formatMessage(msg, isGroup) {
  ...
  if (!msg.fromMe) {
    try {
      ...
      authorName = contact.pushname || contact.name || msg.author; // msg.author es el ID
    } catch (e) {
