...
async function exportCurrentChatFromPage(page) {
  // Aumentamos el timeout a 0 (infinito) porque tu script puede tardar mucho bajando todo el historial
  return await page.evaluate(async () => {
    console.log("[Browser] Iniciando script de usuario original...");

    // --- INICIO DE TU CÓDIGO (Copiado tal cual) ---
    ...
    // (Función download eliminada para evitar descarga fantasma en Docker.
    //  En su lugar, devolvemos los datos al final)
    ...
    function getChatTitle(){
      ...
      if (altHeader){
        const el = altHeader.querySelector('[data-testid="conversation-info-header-chat-title"], span[title], [title]');
        if (el){
          const t = (el.getAttribute("title") || el.textContent || "").trim();
          if (t) return t;
        }
    ...
    // --- LÓGICA PRINCIPAL DE TU SCRIPT (RUN) ---
    async function run(){
      scroller = findScrollContainer();
      if (!scroller){
        console.warn("⚠️ No se encontró contenedor de scroll.");
        scan(); // Escaneo de emergencia
        return;
      }
      ...
      while (running) {
        ...
        // Tu lógica de estancamiento
        stagnation = (added === 0 && after === before) ? (stagnation+1) : 0;
        // console.log(`↑ Ronda ${rounds} | total: ${after} | nuevas: ${added} | estancamiento: ${stagnation}/${maxStagnation}`);
    ...
    // Ejecutamos tu función run() y esperamos a que termine el while
    await run();

    // --- PREPARAR DATOS DE RETORNO (Sustituye a exportTXT) ---
    const lines = [];
    ...
    return {
        ...
        // Pasamos las líneas ya formateadas para que Node solo tenga que escribir
        fullText: lines.join("\n") 
    };
    // --- FIN DE TU CÓDIGO ---
  });
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (exportResult && exportResult.count > 0) {
        ...
        try {
           await db.run("INSERT INTO Exports(sessionId, chatTitle, filePath, exportedAt) VALUES(?,?,?,datetime('now'))", sessionId, title, filePath);
        } catch(e){}
      } else {
        console.log(`[FASE 2] ℹ️ Chat vacío o sin mensajes nuevos.`);
      }

      // Banners
      const hasSyncBanner = await hasSyncInProgressBanner(page);
      if (hasSyncBanner) retryList.push(chat);
