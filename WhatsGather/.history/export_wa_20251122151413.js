...
async function openChatByTitle(page, chatTitle) {
  ...
  // Escribimos con un delay humano
  await page.keyboard.type(cleanTitle, { delay: 50 });
  
  // Esperamos a que WhatsApp filtre (vital)
  await page.waitForTimeout(1500);

  // 2. SELECCIONAR Y ABRIR
  // Pulsamos flecha abajo para asegurar que el foco está en la lista
  await page.keyboard.press("ArrowDown");
  await page.waitForTimeout(300);
  
  console.log("[FASE 2] Pulsando ENTER para abrir...");
  await page.keyboard.press("Enter");

  // 🔴 CORRECCIÓN CRÍTICA: 
  // NO pulsamos Escape aquí. Esperamos a que la UI cambie.
  // Si pulsamos Escape ahora, cancelamos la navegación.
  
  // Damos tiempo para que la vista cambie del "Landing Page" al "Chat"
  await page.waitForTimeout(3000);
  
  // Opcional: Si el buscador sigue sucio, ya lo limpiará el siguiente ciclo
  // o el scraper hará scroll igual.
}
