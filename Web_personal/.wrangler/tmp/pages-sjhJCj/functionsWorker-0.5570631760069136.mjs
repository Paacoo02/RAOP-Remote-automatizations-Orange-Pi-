...
async function verifyJWT(token, secret) {
  ...
  if (typeof json.exp === "number" && Date.now() / 1e3 > json.exp) return null;
  return json;
}
...
async function issueSession(env, sub) {
...
function cookieHeader(env, token) {
...
function getCookie(cookieHeader2, name) {
  const m = cookieHeader2.match(new RegExp(`(?:^|; )${name}=([^;]*)`));
...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  try {
    ...
    if (!await verifyCredentials(env, user, pass)) {
      return new Response(JSON.stringify({ error: "Credenciales inv\xE1lidas" }), { status: 401 });
    }
...
// api/logout.ts
var onRequestPost2 = /* @__PURE__ */ __name(async ({ env }) => {
  return new Response(JSON.stringify({ ok: true }), {
    headers: {
      "content-type": "application/json",
...
// _middleware.ts
var PROTECTED_PREFIXES = ["/dashboard.html", "/apps/"];
...
// ../.wrangler/tmp/pages-sjhJCj/functionsRoutes-0.9189466574832788.mjs
var routes = [
  {
    routePath: "/api/login",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-vGIG07/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-vGIG07/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5570631760069136.mjs.map
