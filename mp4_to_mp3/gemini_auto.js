...
async function dumpOverlayDOM(page, stage = 'unknown') {
  try {
    const data = await page.evaluate(() => {
      const overlay = document.querySelector('.cdk-overlay-container');
      const panes = overlay ? Array.from(overlay.querySelectorAll('.cdk-overlay-pane')) : [];
      const dialogs = overlay ? Array.from(overlay.querySelectorAll('[role="dialog"], .mdc-dialog, .mat-mdc-dialog-container')) : [];
      const buttons = overlay ? Array.from(overlay.querySelectorAll('button')).map(b => ({
        text: (b.innerText || '').trim(),
        dsid: b.getAttribute('data-test-id') || null,
        classes: b.className || '',
      ...
      return {
        ...
        dialogTexts: dialogs.map(d => (d.innerText || '').trim().slice(0, 800)),
        buttons
      };
    });

    ddom(`[${stage}] overlayPresent=${data.overlayPresent} panes=${data.panesCount} dialogs=${data.dialogsCount}`);
    if (data.dialogTexts?.length) ddom(`[${stage}] dialogTexts[0]=`, trimForConsole(data.dialogTexts[0], 1200));
    const btnSample = data.buttons?.slice(0, 8).map(b => `${b.text} | dsid=${b.dsid} | ${b.classes}`).join(' || ');
    if (btnSample) ddom(`[${stage}] overlay buttons sample:`, btnSample);

    // Guardar HTML completo para inspección
    const file = `/tmp/gemini_dialog_dump_${Date.now()}_${stage}.html`;
    const html = `
<!doctype html><meta charset="utf-8">
<h3>Dump @ ${new Date().toISOString()} — stage: ${stage}</h3>
<pre>dialogs=${data.dialogsCount} panes=${data.panesCount}</pre>
<div>${data.overlayHTML || '(no .cdk-overlay-container)'}</div>`;
    await fs.writeFile(file, html);
    ddom(`[${stage}] dump guardado en: ${file}`);

    // Screenshot del overlay (mejor contexto visual)
    try {
      const overlayLoc = page.locator('.cdk-overlay-container');
      if (await overlayLoc.count()) {
        const shot = `/tmp/gemini_dialog_overlay_${Date.now()}_${stage}.png`;
        await overlayLoc.screenshot({ path: shot });
        ddom(`[${stage}] screenshot overlay: ${shot}`);
      }
    } catch {}

  } catch (e) {
    ddom(`[${stage}] dumpOverlayDOM error:`, e.message);
  }
}
